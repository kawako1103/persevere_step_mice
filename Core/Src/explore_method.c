	...
	if(hip_adjustment==1){//前壁かつ左壁あればそれぞれにケツあて(hip_adjustment=1;90前進/180°旋回/遅く90後進/30前進//90°左旋回/遅く90後進/120前進)
		...
		motor_pwm_on();
		  ...
		  trapezoid_accel_backward(1000,100,300,100,90);//90back台形加速の関数遅くね
		    ...
		    motor_pwm_on();
		  trapezoid_accel_forward(2000,100,500,500,120);//120む台形加速の関数
	}else if(hip_adjustment==3){//(前壁かつ右壁)かつ(前壁かつ左壁)あればそれぞれにケツあて(hip_adjustment=3;90前進/180°旋回/遅く90後進/120前進)
		  trapezoid_accel_forward(2000,500,500,100,70);//90む台形加速の関数
			motor_pwm_off();
			motor_pwm_on();
			////////////////////////
...
//			}
			...
			direction=3;//後向き
			if(((x>0)&&((row[y]&(1<<x))==(1<<x))&&((column[x-1]&(1<<y))==(1<<y)))&&(((row[y]&(1<<x))==(1<<x))&&((column[x]&(1<<y))==(1<<y)))){//(前壁かつ右壁)かつ(前壁かつ左壁)あればそれぞれにケツあて(hip_adjustment=3;90前進/180°旋回/遅く90後進/120前進)
				hip_adjustment=3;//両壁あり、両壁けつあてor180°旋回けつあて　※ここ//が&&ではないかと思って変えた
			}
...
//			}
			...
			direction=3;//後向き
			if((((column[x]&(1<<y))==(1<<y))&&((row[y]&(1<<x))==(1<<x)))&&((y>0)&&((column[x]&(1<<y))==(1<<y))&&((row[y-1]&(1<<x))==(1<<x)))){//(前壁かつ右壁)かつ(前壁かつ左壁)あれば(180°旋回して後進→前進 10/23時点)それぞれある方の壁にケツあて(hip_adjustment=3;90前進/180°旋回/90後進/120前進)
				hip_adjustment=3;//　　　　　　　　　　　　　　　　　　　　　　↑ここ//が&&ではないかと思って変えた
			}
		}
...
//			}
			...
			direction=3;//後向き
			if(((y>0)&&((row[y-1]&(1<<x))==(1<<x))&&((column[x]&(1<<y))==(1<<y)))&&((y>0)&&(x>0)&&((row[y-1]&(1<<x))==(1<<x))&&((column[x-1]&(1<<y))==(1<<y)))){//(前壁かつ右壁)かつ(前壁かつ左壁)あればそれぞれにケツあて(hip_adjustment=3;90前進/180°旋回/遅く90後進/120前進)
				hip_adjustment=3;
			}
		}
...
//			}
			...
			direction=3;//後向き
			////
//			if(((x>0)&&(y>0)&&((column[x-1]&(1<<y))==(1<<y))&&((row[y-1]&(1<<x))==(1<<x)))&&((x>0)&&(y>0)&&((column[x-1]&(1<<y))==(1<<y))&&((row[y+1]&(1<<x))==(1<<x)))){//(前壁かつ右壁)かつ(前壁かつ左壁)あればそれぞれにケツあて(hip_adjustment=3;90前進/180°旋回/90後進/120前進)
//				hip_adjustment=3;
//				//前Dist_map[x-1][y]→右Dist_map[x][y+1]→左Dist_map[x][y-1]→後Dist_map[x+1][y]
//			}else{
//				hip_adjustment=0;
//			}
            ///
...
//	int s,k,l;
	...
	//	int v,w;//同上
			...
			//ゴールの隣にきた時に壁があっても認識できるようにする
				if(Dist_map[goal_x][goal_y]==255){//ここが帰り探索の時にちゃんと（0,0）になっているかな？
					aroundgoal=1;//フラグみたいなもん
				}
}
...
//		  hip_drop();
	...
	else if(direction==2){//左向き　にいけ　の時
		  //90 mm直進,90°左旋回,90 mm直進,停止
...
//		  hip_drop();
	...
	else if(direction==3){//後ろ向き
	//90前進,180°左旋回,90前進,停止

		///////2023/10/24の記録　どうやら後ろ向きに方向決定して、hip_adjustment==0と3以外の時に0の時と同じ動きをさせるようにする
		...
		if((hip_adjustment==0)){//条件に合わない時普通に
		  ...
		  hip_drop();
		}else {//上のhip_adjustmentが0と3以外の時は0の時と同じ動きをさせる
			  trapezoid_accel_forward(2000,500,500,100,70);//90む台形加速の関数
			  motor_pwm_off();
			  motor_pwm_on();
			  trapezoid_accel_lturn(2000,100,400,80,180);//左180°曲がる
	...
	if(direction==1){//右向き
		if(hip_adjustment==0){//条件に合わない時普通に
			...
			if((float)g_sensor[1][0]>=WALLREAD_L){//左あり
				control_left_sensor = g_sensor[1][0];//オフセットを調整するためのもの
				//↓左センサーをもとに後ろオフセットを調整
				offset_adjustment_len_lslalom =  -1*(0.000001)*pow(control_left_sensor,3)+0.0021*pow(control_left_sensor,2)-1.3509*control_left_sensor+308.01 - 37.5 ;////84.5(壁から1区画の中心線まで) 機体の中心から左端までが47なので、84.5-47 = 37.5機体が中心にいる時の端から壁まで
				//↑2023/10/25時点
//				offset_adjustment_len_lslalom =  -1*(0.000001)*pow(control_left_sensor,3)+0.0022*pow(control_left_sensor,2)-1.3589*control_left_sensor+309.07 - 37.5 ;////84.5(壁から1区画の中心線まで) 機体の中心から左端までが47なので、84.5-47 = 37.5機体が中心にいる時の端から壁まで
					 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	 	//上の+309.07までが、壁からの距離で測定したセンサー値を元にした近似式(センサー値→壁からの距離)
		      //offset_adjustment_len_lslalom =  -1*(0.000001)*pow(control_left_sensor,3)+0.0022*pow(control_left_sensor,2)-1.3589*control_left_sensor+309.07 + 37 ;//←以前 もしかしたら機体の基板の端から測って他のかも
//				offset_adjustment_len_rslalom =  ((37-0)/(901-595))*(control_left_sensor-595)+0 ;
				///右に曲がるとき, 左センサ(センサ値x)に対しては左向きを正として y = ((37-0)/(901-595))*(x-595)+0 　その後, yだけ左によっているので, yだけオフセットを増やす 両符号
				///R 0:599/30:485/60:275//74:178
				///左に曲がるとき, 右センサ(センサ値x)に対しては右向きを正として y = ((37-0)/(599-388.5))*(x-388.5)+0
			...
			offset_slalom_trapezoid_accel_rturn(500,10000,100,460,80,97,79);///これでいく(20mm全身で左に5度くらい傾いているからあえて回転角度を下げている感じになっている)r(500,10000,100,460,80,98,68);//r(500,10000,100,460,80,93,75)→; (500,17000,100,460,80,90);  (500,20000,100,460,80,90)
//			offset_slalom_trapezoid_accel_rturn(500,10000,100,460,80,96,83);//基本t系にoffsetを調整できるやつで
			non_wall_control_trapezoid_accel_forward(2000,500,500,500,38 - offset_adjustment_len_lslalom);//横壁制御ありver.
			//
//			slalom_trapezoid_accel_rturn(500,10000,100,460,80,91,75);//(500,10000,100,460,80,90); (500,17000,100,460,80,90);  (500,20000,100,460,80,90)
//		    //////////////////////
//			non_wall_control_trapezoid_accel_forward(2000,500,500,500,20 + offset_adjustment_len_rslalom);//横壁制御分入れた
...
//			/////////////////////////
			////横壁制御
				if((float)g_sensor[2][0]>=WALLREAD_R){//右あり
					control_right_sensor = g_sensor[2][0];//オフセットを調整するためのもの
					offset_adjustment_len_rslalom =  -1*(0.000001)*pow(control_right_sensor,3)+0.0019*pow(control_right_sensor,2)-1.1228*control_right_sensor+253.4 - 37.5 ;//84.5(壁から1区画の中心線まで) 機体の中心から左端までが47なので、84.5-47 = 37.5機体が中心にいる時の端から壁まで
					//↑10/25/時点
//					offset_adjustment_len_rslalom =  -7*(0.000001)*pow(control_right_sensor,3)+0.0068*pow(control_right_sensor,2)-2.4156*control_right_sensor+321.2 - 37.5 ;//84.5(壁から1区画の中心線まで) 機体の中心から左端までが47なので、84.5-47 = 37.5機体が中心にいる時の端から壁まで
//					offset_adjustment_len_rslalom =  -7*(0.000001)*pow(control_right_sensor,3)+0.0068*pow(control_right_sensor,2)-2.4156*control_right_sensor+321.2 + 37 ;//←以前
					///右に曲がるとき, 左センサ(センサ値x)に対しては左向きを正として y = ((37-0)/(901-595))*(x-595)+0 　その後, yだけ左によっているので, yだけオフセットを増やす 両符号
					///R 0:599/30:485/60:275//74:178
					///左に曲がるとき, 右センサ(センサ値x)に対しては右向きを正として y = ((37-0)/(599-388.5))*(x-388.5)+0
				}else{
...
void return_slalom_continual_adachi_method(void){
	//
//	//歩数マップの初期化これは足立法の繰り返しでは入れてはいけないきがす、、
//	for(x=0;x<=15;x++){
//		for(y=0;y<=15;y++){
...
//	}
	  ...
	  ///帰り探索  ////今いるゴール位置をスタートとして、ゴールを(0.0)にする　zと壁情報は受け継ぐ
...
//	  motor_pwm_on();
	  trapezoid_accel_forward(2000,100,500,500,90);//90む台形加速の関数
      //ゴールしたら区画の中心にいるはずなので、90進んで壁読むところからスタート
...
//	  trapezoid_accel_forward(2000,100,500,500,90);//90進む台形加速の関数
	  ...
	  //座標更新//ゴールは必ず入ったますで止まる。そのあと半区画前進した次のマスからスタート
	  if(z==0||z==4){//北に対して、、
		  x=x;
		  y=y+1;
		  if(z==4){
...
//			printf("プログラムの初めx=%d,y=%d,z=%d\n\r",x,y,z);
			//終了判定
			if((x==goal_x)&&(y==goal_y)){//この関数においては、ゴールが(0,0) 帰り探索用な。ゴールに来たら終了 ///帰り探索だから(0,0)がゴール
					trapezoid_accel_forward(2000,500,500,100,90);//90む台形加速の関数
					motor_pwm_off();
					HAL_Delay(1000);//1秒経ってから励磁解除
...
//帰り探索用 戻り横壁制御スラローム連続足立法
void return_side_added_slalom_continual_adachi_method(void){
	//
//	//歩数マップの初期化これは足立法の繰り返しでは入れてはいけないきがす、、
//	for(x=0;x<=15;x++){
//		for(y=0;y<=15;y++){
